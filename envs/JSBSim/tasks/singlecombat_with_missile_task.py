...
class HierarchialHybridSingleCombatTask(HierarchicalSingleCombatTask): ##for action overloading
    def __init__(self, config: str):
        HierarchicalSingleCombatTask.__init__(self, config)
        self.reward_functions = [
            PostureReward(self.config),
    ...
    def normalize_action(self, env, agent_id, obs, rnn_states, action, action_representation): #must make actionrepresentation to come here
        ...
        return HierarchicalSingleCombatTask.normalize_action(self, env, agent_id, \
