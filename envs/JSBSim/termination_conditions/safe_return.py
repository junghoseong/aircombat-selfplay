...
class SafeReturn(BaseTerminationCondition):
    ...
    def get_termination(self, task, env, agent_id, info={}):
        ...
        if env.agents[agent_id].is_shotdown:
            ...
            info['done_condition'] = Fore.LIGHTRED_EX + f'{agent_id} has been shot down! Total Steps={env.current_step}'
            return True, False, info

        elif env.agents[agent_id].is_crash:
            self.log(f'{agent_id} has crashed! Total Steps={env.current_step}')
            info['done_condition'] = Fore.LIGHTRED_EX + f'{agent_id} has crashed! Total Steps={env.current_step}'
            return True, False, info
